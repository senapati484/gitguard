...
export async function getUserInstallationDocs(
  ...
  const fetchDocs = async (): Promise<QueryDocumentSnapshot[]> => {
    try {
      ...
      console.warn(
        `[installations] Notice: Could not fetch user installations:`,
  ...
  const timeoutPromise = new Promise<QueryDocumentSnapshot[]>((resolve) =>
    setTimeout(() => resolve([]), 3000)
  );

  return Promise.race([fetchDocs().catch(() => []), timeoutPromise]);
}
